...
class _FeedPageState extends State<FeedPage> {
  ...
  Future<void> _toggleLike(int postId) async {
    ...
    try {
      ...
      if (isLiked) {
        await supabase.from('likes').delete().match({'user_id': uid, 'post_id': postId});
      } else {
  ...
  Future<void> _followOrUnfollow(String authorId) async {
    ...
    if ((f as List).isEmpty) {
      ...
      await supabase.from('follows').delete().match({'follower_id': me, 'followee_id': authorId});
    }
    await _load();
  }
            ...
            return Card(
              ...
              child: Column(
                ...
                children: [
                  ListTile(
                    leading: CircleAvatar(
                      backgroundImage: (p['avatar_url'] as String?)?.isNotEmpty == true
                          ...
                          : null,
                      child: (p['avatar_url'] as String?)?.isNotEmpty == true ? null : const Icon(Icons.person),
                    ),
                  ...
                  CachedNetworkImage(imageUrl: p['image_url'], fit: BoxFit.cover),
                  Padding(
